...
const dialogbox = document.querySelector("#course-info");

// function to show modal with course details
function showCourseModal(course) {
    // fill the dialog content
    dialogbox.innerHTML = `
    <div class="modal-content">
        <button id="close-modal">X</button>
        <h2>${course.subject} ${course.number}: ${course.title}</h2>
        <p><strong>Credits:</strong> ${course.credits}</p>
        <p><strong>Certificate:</strong> ${course.certficate}</p>
    ...
    // close button (using querySelector because button was created above in the function rather than the HTML)
...
function renderCourses(courseArray) {
    ...
    courseArray.forEach(course => {
        ...
        detailsBtn.addEventListener("click", () => showCourseModal(course));
    });
